...
def specify_resolution(image_list, depth_list, score_list, mask_list, resolution: tuple=(400,400)):
    resize_image = []
    resize_depth = []
    resize_score = []
    resize_mask = []
    # print(resolution)
    for image, depth, score, mask in zip(image_list, depth_list, score_list, mask_list):
        image = cv2.resize(image, (resolution[0], resolution[1]), interpolation=cv2.INTER_CUBIC)
        depth = cv2.resize(depth, (resolution[0], resolution[1]), interpolation=cv2.INTER_CUBIC)
    ...
    return resize_image, resize_depth, resize_score, resize_mask
