...
def plot_sample_cv2(names, imgs, depths, scores_: dict, gts, save_folder=None):
    # get subplot number
    ...
    # save imgs
    for idx in tqdm(range(total_number), ncols=100, desc=f'Saving results to {save_folder}'):
        cv2.imwrite(os.path.join(save_folder, f'{names[idx]}_ori.jpg'), imgs[idx])
        cv2.imwrite(os.path.join(save_folder, f'{names[idx]}_depth.jpg'), depths[idx])
