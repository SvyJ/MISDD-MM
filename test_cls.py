...
def test(model,
    ...
    for (img, pc, mask, label, name, img_type) in tqdm(dataloader, ncols=100, desc=f'{args.dataset}/{args.class_name}, {args.k_shot} shot'):

        img = [model.img_transform(Image.fromarray(f.numpy())) for f in img]
        img = torch.stack(img, dim=0)

        for d, n, l, m in zip(img, name, label, mask):
        ...
        img = img.to(device)
        score_img, score_map = model(img, 'cls')
...
def get_args():
    ...
    parser.add_argument("--backbone", type=str, default="ViT-B-16-plus-240", choices=['ViT-B-16-plus-240', 'ViT-B-16'])
