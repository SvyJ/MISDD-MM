...
def fit(model,
    ...
    for (img, pc, depth, mask, label, name, img_type, missing_flag) in tqdm(train_data, ncols=100, desc=f'{args.dataset}/{args.class_name}, missing {args.missing_type}-{args.missing_rate}, building feature gallery:'):
        ...
        # torch.Size([2, 3, 240, 240]) torch.Size([2, 3, 240, 240]) torch.Size([2, 3, 240, 240])
        # print(img.shape, pc.shape, depth.shape)
    ...
    for epoch in range(args.Epoch):
        ...
        for (img, pc, depth, mask, label, name, img_type, missing_flag) in tqdm(train_data, ncols=100, desc=f'{args.dataset}/{args.class_name}, missing {args.missing_type}-{args.missing_rate}, Epoch {epoch}/{args.Epoch}, training'):
            ...
            missing_flag = [m for m in missing_flag]
            ...
            _, img_feature_map, _, _ = model.encode_image_missing(img, all_prompts_image, missing_flag)
            ...
            img_trip_loss = criterion_tip(img_feature_map, img_normal_text_features_ahchor, img_abnormal_text_features_ahchor)
            img_l_pos = torch.einsum('nic,cj->nij', img_feature_map, img_normal_text_features_ahchor.transpose(0, 1))
            img_l_neg_v2t = torch.einsum('nic,cj->nij', img_feature_map, img_abnormal_text_features.transpose(0, 1))
            img_logits_v2t = torch.cat([img_l_pos, img_l_neg_v2t], dim=-1) * model.model.logit_scale
            img_target_v2t = torch.zeros([img_logits_v2t.shape[0], img_logits_v2t.shape[1]], dtype=torch.long).to(device)
            img_loss_v2t = criterion(img_logits_v2t.transpose(1, 2), img_target_v2t)
            img_loss = img_loss_v2t + img_trip_loss + img_loss_match_abnormal * args.lambda1

            
            _, depth_feature_map, _, _ = model.encode_image_missing(depth, all_prompts_depth, missing_flag)
            ...
            depth_trip_loss = criterion_tip(depth_feature_map, depth_normal_text_features_ahchor, depth_abnormal_text_features_ahchor)
            depth_l_pos = torch.einsum('nic,cj->nij', depth_feature_map, depth_normal_text_features_ahchor.transpose(0, 1))
            depth_l_neg_v2t = torch.einsum('nic,cj->nij', depth_feature_map, depth_abnormal_text_features.transpose(0, 1))
            depth_logits_v2t = torch.cat([depth_l_pos, depth_l_neg_v2t], dim=-1) * model.model.logit_scale
            depth_target_v2t = torch.zeros([depth_logits_v2t.shape[0], depth_logits_v2t.shape[1]], dtype=torch.long).to(device)
            depth_loss_v2t = criterion(depth_logits_v2t.transpose(1, 2), depth_target_v2t)
        ...
        if best_result_dict is None:
            ...
            print(f'===========================Pixel-AUROC: {result_dict["p_roc"]}, AUPRO: {result_dict["pro_auc"]}')
        ...
        wandb.log({
            'Pixel-AUROC': best_result_dict['p_roc'],
            'AUPRO': best_result_dict['pro_auc']
        })
...
def main(args):
    ...
    wandb.init(
        # project = 'Prompt-RGB_PC-SEG',
        project = 'Prompt-RGB_DEPTH-SEG-Missing-V3',
